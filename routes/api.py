...
from options.executor import (
    ...
    get_executor_log as get_options_executor_log,
)
...
@api_bp.route("/api/futures/executor/run", methods=["POST"])
@require_admin_auth
def api_futures_executor_run():
    try:
...
@api_bp.route("/api/futures/executor/monitor", methods=["POST"])
@require_admin_auth
def api_futures_executor_monitor():
    try:
        result = run_futures_position_monitor()
        return jsonify(result), (200 if result.get("ok") else 500)
    except Exception as exc:
        return jsonify({"ok": False, "error": str(exc)}), 500


@api_bp.route("/api/futures/executor/log", methods=["GET"])
@require_api_auth
def api_futures_executor_log():
    try:
        limit = max(1, min(200, int(request.args.get("limit") or 50)))
        return jsonify({"ok": True, "log": get_executor_log(limit)})
    except Exception as exc:
        return jsonify({"ok": False, "error": str(exc)}), 500


@api_bp.route("/api/options/executor/run", methods=["POST"])
@require_admin_auth
def api_options_executor_run():
    try:
...
@api_bp.route("/api/options/executor/monitor", methods=["POST"])
@require_admin_auth
def api_options_executor_monitor():
    try:
        result = run_options_position_monitor()
        return jsonify(result), (200 if result.get("ok") else 500)
    except Exception as exc:
        return jsonify({"ok": False, "error": str(exc)}), 500


@api_bp.route("/api/options/executor/log", methods=["GET"])
@require_api_auth
def api_options_executor_log():
    try:
        limit = max(1, min(200, int(request.args.get("limit") or 50)))
