...
def get_today_realized_pnl():
    ...
    baseline = state["baseline_realized_pnl"]
...
def process_alert(data):
    ...
    try:
        ...
        if signal_type in {
            ...
            result = {
                "ok": False,
                "reason": f"daily_loss_limit_reached pnl={today_pnl}",
            }
            update_last_webhook(
                order_id=order_id,
                action=action,
        ...
        if signal_type == "CORE_BUY_WINDOW":
            ...
            amount = float(
                summary.get("assets", {}).get(product_id, {}).get("allowed_buy_usd", 0.0) or 0.0
            )
        ...
        if signal_type == "TRIM_TO_TARGET":
            ...
            summary = portfolio_summary(snapshot)
            trim_usd = float(
                summary.get("assets", {}).get(product_id, {}).get("required_trim_usd", 0.0) or 0.0
            )
...
def dashboard():
    ...
    for item in plan.get("trims", []):
        max_wt_display = ""
        if "max_weight" in item:
            max_wt_display = f"{float(item.get('max_weight', 0.0)):.3f}"
    ...
    if not buy_rows:
        buy_rows = """
        <tr>
            <td colspan="5">No buy opportunities right now.</td>
        </tr>
        """
    ...
    if not trim_rows:
        trim_rows = """
        <tr>
            <td colspan="5">No trim opportunities right now.</td>
        </tr>
        """
    ...
    <head>
        ...
        <style>
            ...
            .row-core td:first-child {{
                color: #93c5fd;
                font-weight: bold;
            }}
            .row-sat td:first-child {{
                color: #c4b5fd;
                font-weight: bold;
            }}
            .mono {{
    ...
    <body>
        ...
        <div class="grid">
            <div class="card">
                <div class="label">Trading Enabled</div>
                <div class="value"><span class="badge {badge_class(TRADING_ENABLED, 'bool')}">{TRADING_ENABLED}</span></div>
            </div>
            <div class="card">
                <div class="label">Buy Allowed</div>
                <div class="value"><span class="badge {badge_class(buy_ok, 'bool')}">{buy_ok}</span></div>
            </div>
            <div class="card">
                <div class="label">Market Regime</div>
                <div class="value"><span class="badge {badge_class(market_regime, 'regime')}">{market_regime}</span></div>
            </div>
            <div class="card">
                <div class="label">Today Realized PnL</div>
                <div class="value"><span class="badge {badge_class(pnl, 'pnl')}">${pnl:,.2f}</span></div>
            </div>
            ...
            <div class="card">
                ...
                <div class="value">${usd_cash:,.2f}</div>
            </div>
            <div class="card">
                <div class="label">Portfolio Peak</div>
                <div class="value">${peak:,.2f}</div>
            </div>
            <div class="card">
                <div class="label">Drawdown</div>
                <div class="value"><span class="badge {badge_class(drawdown, 'drawdown')}">{drawdown:.2%}</span></div>
            </div>
        </div>
